            ...
            if (msgType.equals(MessageUtil.REQ_MESSAGE_TYPE_TEXT)) {
                ...
                switch (subscriber_info.getSubscriber_Function()) {
                    case "register":
                     // 吕嘉铭 男 22 13611774556
                        try {
                            ...
                            if (tag == 0) {  // 第一次注册
                                ...
                                Member_Info new_member_info = new Member_Info(
                                        keywords[0], keywords[1], Integer.parseInt(keywords[2]), keywords[3], db.getDate(0), fromUserName, false);
                        ...
                        if (content.startsWith("yzm")) { // yzm 1
                    ...
                    case "addbook": // ISBN Library_ID
                        if (keywords.length == 2) {
                            String ADD_ISBN = keywords[0];
                            int Library_id = Integer.parseInt(keywords[1]);
                            ...
                            respContent = "输入格式有误。请仿照\"ISBN 图书馆代号\"格式输入。";
                        }
                        break;
                ...
                if (content.startsWith("Worker") || content.startsWith("worker")) { // Worker 1 吕嘉铭
                    ...
                    if (tag == 0) {
                        ...
                        respContent = db.getMember_Info(fromUserName) != null && db.getWoker_InfobyfromUserName(fromUserName) != null ? "成功与员工账号绑定，输入\"Worker 员工编号 员工姓名\"即可登录。" : "成功与员工账号绑定，请点击\"登录/注册\"按钮进行登录。";
                    } else if (tag == 2) {
                        TagManager.batchtagging(fromUserName, "Worker");
                        respContent = "员工【" + worker_info.getWorker_Name() + "】登录成功";
                    }
                ...
                if (eventType.equals(MessageUtil.EVENT_TYPE_SUBSCRIBE)) {
                    ...
                    if(User_Info.get("sex") == 1)  {Subscriber_Sex = "男";}
                    else if (User_Info.get("sex") == 2) {Subscriber_Sex = "女";}
                    Subscriber_Info new_subscriber_info = new Subscriber_Info(
                            ...
                            (String)User_Info.get("subscribe_time"), (String)User_Info.get("tagid_list"));
